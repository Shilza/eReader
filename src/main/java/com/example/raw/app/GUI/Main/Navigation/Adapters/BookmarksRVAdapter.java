package com.example.raw.app.GUI.Main.Navigation.Adapters;
...
import com.example.raw.app.GUI.RVViewHolder;
...
public class BookmarksRVAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        switch (holder.getItemViewType()) {
            ...
            case 1:
                extendedViewHolderPreparing(position);
                break;
        }
    }
    ...
    private void extendedViewHolderPreparing(int position) {
    ...
    private void iniRemovingBookmarksDialog() {
        ...
        ad.setPositiveButton("Да", (DialogInterface dialog, int arg1) -> {
            selectedBook.getBookmarks().clear();

            for (Pair<Book, Integer> pair : books)
                if (pair.first.equals(selectedBook)) {
                    books.remove(pair);
                    break;
                }
        ...
        ad.setNegativeButton("Нет", (DialogInterface dialog, int arg1) -> {
        });
    }
}
